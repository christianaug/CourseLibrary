...
namespace CourseLibrary.API.Services
{
    public class CourseLibraryRepository : ICourseLibraryRepository, IDisposable
    {
        ...
        public PagedList<Author> GetAuthors(AuthorsResourceParameters authorsResaourceParameter)
		{
            ...
            //the execution of this query doesnt actually happen until its itterated over
            var collection = _context.Authors as IQueryable<Author>; 

            if (!string.IsNullOrWhiteSpace(authorsResaourceParameter.MainCategory))
			{
                var mainCategory = authorsResaourceParameter.MainCategory.Trim();
                collection = collection.Where(a => a.MainCategory == mainCategory);
            }

            if (!string.IsNullOrWhiteSpace(authorsResaourceParameter.SearchQuery))
			{
                var searchQuery = authorsResaourceParameter.SearchQuery.Trim();
            ...
            if (!string.IsNullOrWhiteSpace(authorsResaourceParameter.OrderBy))
			{
                ...
                var authorMappingDictionary = _propertyMappingService.GetPropertyMapping<AuthorDto, Author>();

				collection = collection.ApplySort(authorsResaourceParameter.OrderBy, authorMappingDictionary);
			}
